...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore)
{
    ...
    // 步骤1：找出和当前帧具有公共单词的所有关键帧（不包括与当前帧链接的关键帧）
    {
        ...
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)//访问当前帧的单词
        {
            ...
            {
                KeyFrame* pKFi=*lit;
                //todo 先计算了semantic_index，不满足的直接pass
                if( __builtin_popcount(pKFi->semantic_index ^ pKF->semantic_index)>3){
                    // std::cout<<"pkfi index= "<<bitset<16>(pKFi->semantic_index)<<std::endl;
                    // std::cout<<"pkf index= "<<bitset<16>(pKF->semantic_index)<<std::endl;
                    // std::cout<<"Semantic_index distence = "<<index_dist<<"  , drop this keyframe..."<<std::endl;
                    continue;
                }
    ...
    // 步骤3：遍历所有闭环候选帧，挑选出共有单词数大于minCommonWords且单词匹配度大于相邻关键帧计算得到的minScore存入lScoreAndMatch
    //会挑选出每个候选关键帧以及其相连共识最高的10个相连关键帧合并为一组，累计计算为该帧得分
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
    ...
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        float bestScore = it->first;// 该组最高词袋计算分数
