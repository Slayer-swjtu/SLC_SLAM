...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, ros::ServiceClient& client)
{
    if(mSensor!=RGBD)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageRGBD(im,depthmap,timestamp,client);
