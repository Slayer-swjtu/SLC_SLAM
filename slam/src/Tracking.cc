...
// 2、进行tracking过程
// 输出世界坐标系到该帧相机坐标系的变换矩阵
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, ros::ServiceClient& client)
{
    mImRGB = imRGB;
    mImGray = imRGB;
    mImDepth = imD;
    ...
    if((fabs(mDepthMapFactor-1.0f)>1e-5) || mImDepth.type()!=CV_32F){
        ROS_INFO("do the convert, mDepthMapFactor=%f",mDepthMapFactor);
        // std::cout<<"before convert:------- "<<std::endl;
        // std::cout<<mImDepth<<std::endl;
...
void Tracking::Track()
{
    ...
    else// 步骤2：跟踪
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            {
                // BOW搜索，PnP求解位姿
                //todo 这里可以更改为使用点云配准的方式来出解
                //todo 和视觉不同的是，他可能不要那么具体的特征，在建图的部分应该很难重定位成功。
                //todo 但点云总觉得会更加鲁棒？对比试验应该也好做。
                bOK = Relocalization();
            }
        }
        else
...
void Tracking::Track_ros(ros::ServiceClient& client)
{
    // track包含两部分：估计运动、跟踪局部地图
 ...
 */
void Tracking::CreateNewKeyFrame_ros(ros::ServiceClient& client)
{
    ...
    //todo 所以这里可以传给语义提取模块，传rgb+depth，阻塞返回语义符
    //todo depth里已经被转化成了浮点
    boost::timer t_;
    esa::rgbd srv;
    sensor_msgs::ImagePtr rgb_ptr= cv_bridge::CvImage(std_msgs::Header(), "bgr8",  this->mImRGB).toImageMsg();
    srv.request.rgb_data=*rgb_ptr;
    srv.request.height=this->ori_depth.rows;
    srv.request.width=this->ori_depth.cols;
    srv.request.depth_data=vector<float>(this->ori_depth.reshape(1,1));
    std::cout<<"msg package cost time: "<<t_.elapsed()<<"s"<<std::endl;
    if(client.call(srv)){
        std::cout<<"srv return success"<<std::endl;
        auto desc = srv.response.desc;
        pKF->semantic_desc=cv::Mat(desc.size()/4,4,CV_32S,desc.data());
        std::cout<<"keyframe semantic desc: \n"<<pKF->semantic_desc<<std::endl;
        auto cvImgPtr=cv_bridge::toCvCopy(srv.response.pred_result,"rgb8");
        cv::Mat cvColorImgMat=cvImgPtr->image;
        std::cout<<"srv cost time: "<<t_.elapsed()<<"s"<<std::endl;
        cv::imshow("image", cvColorImgMat);
        cv::waitKey(5);
        // 是否要保存在堆内存，kf类里存指针防止栈溢出？
        int count=0;
        pKF->semantic_index=0;
        for(auto n: desc){
            if(count%4==0){
                for(auto flag: objects_15){
                    if(flag==n){
                        pKF->semantic_index+=1;
                        break;
                    }
                }
                pKF->semantic_index<<=1;   
        ...
        std::cout<<"srv cost time: "<<t_.elapsed()<<"s"<<std::endl;
        std::cout<<"index_code: "<<bitset<16>(pKF->semantic_index)<<std::endl;
        ...
        //todo 可以在这里更改渲染
        mpPointCloudMapping->insertKeyFrame( pKF, cvColorImgMat, this->mImDepth  ,idk,vpKFs);
        mnLastKeyFrameId = mCurrentFrame.mnId;
        mpLastKeyFrame = pKF;
    }
    else{
